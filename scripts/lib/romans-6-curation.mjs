...
export function loadRomansSixCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-6-curated.json"));
}

export function applyRomansSixCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansSixCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 6 || chapter.verses?.length !== 23) {
    throw new Error("Expected Romans 6 content with 23 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 6 entry for ${verse.verse}`);
