...
export function loadRomansFiveCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-5-curated.json"));
}

export function applyRomansFiveCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansFiveCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 5 || chapter.verses?.length !== 21) {
    throw new Error("Expected Romans 5 content with 21 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 5 entry for ${verse.verse}`);
