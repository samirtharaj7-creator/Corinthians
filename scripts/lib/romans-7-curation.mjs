...
export function loadRomansSevenCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-7-curated.json"));
}

export function applyRomansSevenCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansSevenCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 7 || chapter.verses?.length !== 25) {
    throw new Error("Expected Romans 7 content with 25 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 7 entry for ${verse.verse}`);
