...
export function loadRomansElevenCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-11-curated.json"));
}

export function applyRomansElevenCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansElevenCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 11 || chapter.verses?.length !== 36) {
    throw new Error("Expected Romans 11 content with 36 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 11 entry for ${verse.verse}`);
