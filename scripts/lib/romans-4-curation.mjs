...
export function loadRomansFourCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-4-curated.json"));
}

export function applyRomansFourCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansFourCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 4 || chapter.verses?.length !== 25) {
    throw new Error("Expected Romans 4 content with 25 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 4 entry for ${verse.verse}`);
