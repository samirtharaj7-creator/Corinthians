...
export function loadRomansEightCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-8-curated.json"));
}

export function applyRomansEightCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansEightCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 8 || chapter.verses?.length !== 39) {
    throw new Error("Expected Romans 8 content with 39 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 8 entry for ${verse.verse}`);
