...
export function loadRomansTenCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-10-curated.json"));
}

export function applyRomansTenCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansTenCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 10 || chapter.verses?.length !== 21) {
    throw new Error("Expected Romans 10 content with 21 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 10 entry for ${verse.verse}`);
