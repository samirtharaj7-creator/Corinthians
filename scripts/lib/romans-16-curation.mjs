...
export function loadRomansSixteenCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-16-curated.json"));
}

export function applyRomansSixteenCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansSixteenCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 16 || chapter.verses?.length !== 27) {
    throw new Error("Expected Romans 16 content with 27 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 16 entry for ${verse.verse}`);
