...
export function loadRomansFifteenCuration(root = process.cwd()) {
  return readJson(join(root, "scripts", "data", "romans-15-curated.json"));
}

export function applyRomansFifteenCuration(chapter, sourceIds, root = process.cwd()) {
  const curation = loadRomansFifteenCuration(root);
  const verseMap = new Map(curation.verses.map((entry) => [entry.verse, entry]));

  if (chapter.chapterNumber !== 15 || chapter.verses?.length !== 33) {
    throw new Error("Expected Romans 15 content with 33 verses.");
  }

  const verses = chapter.verses.map((verse) => {
    const entry = verseMap.get(verse.verse);
    if (!entry) throw new Error(`Missing curated Romans 15 entry for ${verse.verse}`);
